...
def prepare_text_for_lda(text):
    if not (isinstance(text, float) and math.isnan(text)) and len(text)>0:
        ...
        #print(type(tokens))
        return tokens
"""apply funziona su una base riga / colonna di un DataFrame,
    applymap funziona in base agli elementi su un DataFrame
    e map funziona in base agli elementi di una serie."""
def tokenize_worker(all_news, result):
    print "Inizio ",os.getpid()
    #all_news['news'] = all_news['news'].map(lambda row: prepare_text_for_lda(row))
    tok = all_news['news'].map(lambda row: prepare_text_for_lda(row))
    result.put(tok.values)
...
if __name__ =='__main__':
    ...
    all_news = all_news.loc[~all_news['date'].str.contains('ago')]
    all_news['date'] = all_news['date'].map(lambda row: pd.to_datetime(doubledecode(row, True), format='%b %d, %Y', errors='coerce'))
    all_news = all_news.dropna()
    all_news['date'] = all_news['date'].map(lambda row: pd.to_datetime(row.strftime('%d-%m-%Y'), format='%d-%m-%Y'))
    all_news = all_news.sort_values(by='date', ascending=False)
    print(all_news['date'])
    #display(commodities_news.select("*"))
    dim_dataframe = len(all_news)/2
    processes = []
    for i in range(0,1):
        p = Process(target=tokenize_worker,args=(all_news.iloc[i*dim_dataframe:(i+1)*dim_dataframe], result))
        p.start()
        processes.append(p)

    #Inizializzo l'ultimo processo per prendere il dataFrame fino la fine
    p = Process(target=tokenize_worker, args=(all_news.iloc[1 * dim_dataframe:], result))
    p.start()
    processes.append(p)

    res = [result.get() for p in processes]
    print res
    print time.time() - t1
    #t2 = time.time()
    #text_data = []
   ...
   # ldamodel = gensim.models.ldamulticore.LdaMulticore(corpus, num_topics = NUM_TOPICS, id2word=dictionary, passes=100, workers=32)
